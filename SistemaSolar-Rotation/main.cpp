...
class MyIHClass : public VART::ViewerGlutOGL::IdleHandler
{
    public:
        ...
        virtual void OnIdle() {
            ...
            luaRot->MakeZRotation(redRadians*5);
...
int main(int argc, char* argv[])
{
   ...
   //Sol
    ...
    terraTranslation.MakeTranslation(15,0,0);
    if(texture.LoadFromFile("earth.jpg")){
        material.SetTexture(texture);
        terra.SetMaterial(material); 
        terra.SetDescription("Terra");
        terraRotation.AddChild(terraTranslation);
        terraTranslation.AddChild(terra);
    ...
    luaTranslation.MakeTranslation(5,0,0);
    if(texture.LoadFromFile("moon.png")){
        material.SetTexture(texture);
        lua.SetMaterial(material);
        lua.SetDescription("Lua");
        terraTranslation.AddChild(luaRotation);
        luaRotation.AddChild(luaTranslation);
        luaTranslation.AddChild(lua);
    }
    else
    {
        cout << "Could not open texture file. Check if the image format is supported, "
